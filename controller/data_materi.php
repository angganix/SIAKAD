...
$fieldSort    = array("judul", "A.tahun_ajaran", "wali_kelas", "nama_kelas", "last_edit","id","is_ok");
$fieldSearch  = array("judul", "A.tahun_ajaran", "E.nama", "nama_kelas", "last_edit");
$stringSearch = "";
...
if($kelas !== "0" AND $kelas !== ""){
  $filter_kelas = "AND D.id=".$kelas;
}
...
if ($act == "getAll") {
    ...
    $tbl_select .= "A.file, B.tahun_ajaran, C.nama_kurikulum, D.nama_kelas, E.nama AS wali_kelas, A.status";

    $tbl_join = "pembelajaran A LEFT JOIN akademik B ON A.tahun_ajaran = B.id ";
    $tbl_join .= "LEFT JOIN kurikulum C ON A.kurikulum = C.id ";
    $tbl_join .= "LEFT JOIN kelas D ON A.kelas = D.id ";
    $tbl_join .= "LEFT JOIN data_guru E ON D.wali_kelas = E.id ";

    $query     = $cls->getArray($tbl_select, $tbl_join, "($criteria) AND A.kelas != 0 $filter_kelas $filter_telaah $filter_status ORDER BY $fieldSort[$sortBy] $sortDir LIMIT $offset, $perPage");
    $get_count = $cls->getCount($tbl_select, $tbl_join, "($criteria) AND A.kelas != 0 $filter_kelas $filter_telaah $filter_status");
