...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active">Pembelajaran Bidang Studi</li>
    </ol>
</section>
...
<section class="content">
    <div class="box box-widget">
        <div class="box-header with-border">
            <h6 class="box-title">List Materi Bidang Studi</h6>
            <div class="box-tools pull-right">
                <button type="button" class="btn btn-primary btn-xs bg-orange" onclick="filterToggle()"><i
                        class="fa fa-filter fa-fw"></i> Filter</button>
                <button type="button" class="btn btn-box-tool btnAdd" onclick="addNew();"><i
                        class="fa fa-plus fa-fw"></i> Tambah</button>
            </div>
        </div>
        <div class="box-header with-border" id="box-filter" style="border-top: 1px solid #eee;display: none;">
            <div class="row">
                ...
                <div class="col-md-3">
                    <select class="form-control input-sm select2" style="display: inline-block;" id="txtFilterJurusan">
                    </select>
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    <button type="button" class="btn btn-primary btn-sm bg-green" onclick="applyFilter();"><i
                            class="fa fa-search fa-fw"></i> Apply Filter</button>
                    <button type="button" class="btn btn-primary btn-sm bg-orange" onclick="resetFilter();"><i
                            class="fa fa-refresh fa-fw"></i> Reset Filter</button>
                </div>
            </div>
        </div>
                ...
                <table class="table table-responsive table-bordered table-hover dataGrid" id="dataGrid">
                    <thead>
                        <tr>
                            ...
                            <th style="text-align: center;width:16%">Pengajar</th>
                            <th style="text-align: center;width:9%">B. Studi</th>
                            <th style="text-align: center;width:9%">Update</th>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                    ...
                    <div class="row">
                        <div class="col-md-5">
                            <div class="form-group">
                                ...
                                <select class="form-control input-sm select2" id="id_format" name="id_format"
                                    onchange="changeType();">
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                <div id="type_opsi" hidden>
                                    <select class="form-control input-sm" name="format_value_opsi"
                                        id="format_value_opsi">
                                    </select>
                                </div>

                                <div id="type_text" hidden>
                                    <input type="text" class="form-control input-sm" name="format_value_text"
                                        id="format_value_text">
                                </div>
                            </div>

                            <div class="form-group">
                                <label class="small-label" for="kelas">Kelas</label>
                                <select class="form-control input-sm select2" id="kelas_bidang_studi"
                                    name="kelas_bidang_studi"></select>
                            </div>
                        ...
                        <div class="col-md-5 pull-right">
                            <div class="form-group">
                                ...
                                <select class="form-control input-sm select2" id="tahun_ajaran"
                                    name="tahun_ajaran"></select>
                            </div>

                            <div class="form-group">
                                <input type="hidden" name="kurikulum" id="kurikulum" />
                                <label class="small-label" for="kurikulum">Bidang Studi</label>
                                <select class="form-control input-sm select2" id="id_jurusan"
                                    name="id_jurusan"></select>
                            </div>
                            <div>
                                <label class="small-label" for="document"
                                    style="margin-bottom: 15px !important;">File</label>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-success btn-sm"><i class="fa fa-check fa-fw"></i>
                        Simpan</button>
                </div>
            </div>
        </div>
...
<div class="modal fade" id="modal-status">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button onclick="changeStatusPembelajaran()" type="button" class="btn btn-xs btn-success bg-green"><i
                        class="fa fa-check fa-fw"></i> Simpan</button>
            </div>
        </div>
    </div>
</div>
...
function resetFilter() {
    ...
    $("#txtFilterJurusan").val("0").trigger("change");
...
var tabel = $("#dataGrid").DataTable({
    ...
    paging: true,
    "ajax": {
        url: "controller/data_materi_keterampilan.php",
        type: "post",
        data: function(req) {
            req.act = "getAll";
            req.id_jurusan = $("#txtFilterJurusan").val();
            req.kelas_bidang_studi = $("#txtFilterKelas").val();
    ...
    columns: [{
            ...
            render: function(data) {
                ...
                if (data.file_extension === "") {
                    ...
                    return data.judul + "<span class='pull-right label " + color +
                        "' style='padding-bottom:4px;'>" + ext + "</span>";
                }
        ...
        },
        {
            ...
            render: function(data) {
                ...
                if (data.file === "" || data.file === null || data.file_id === "" || data
                    .file_extension === "") {
                    ...
                    var preview_file = "https://docs.google.com/" + doc_type + "/d/" + data.file_id +
                        "/preview";
                    var edit_file = "https://docs.google.com/" + doc_type + "/d/" + data.file_id +
                        "/edit";

                    return "<a class='btn btn-success btn-xs bg-green btn-file' href='" + preview_file +
                        "' target='_blank'><i class='fa fa-eye fa-fw'></i></a> " +
                        "<a class='btn btn-success btn-xs bg-blue btn-file' href='" + edit_file +
                        "' target='_blank'><i class='fa fa-pencil fa-fw'></i></a> " +
                        "<a style='display:none;' class='btn btn-success btn-xs bg-orange btn-file' href='#' onclick='deleteFiles(" +
                        data.id + ",\"" + data.file_id + "\")'><i class='fa fa-times fa-fw'></i></a>";
        ...
        },
        {
            ...
            render: function(data) {
                ...
                if (data.is_ok === "1") {
                    states +=
                        "<button type='button' class='btn btn-success btn-xs bg-green' onclick='changeState(" +
                        data.id + ",0)'><i class='fa fa-check fa-fw'></i> Sudah</button>";
                } else {
                    states +=
                        "<button type='button' class='btn btn-warning btn-xs bg-orange' onclick='changeState(" +
                        data.id + ",1)'><i class='fa fa-times fa-fw'></i> Belum</button>";
                }
        ...
        },
        {
            ...
            render: function(data) {
                ...
                if (data === "-" || data === null) {
                    ...
                    stt += "<button type='button' class='btn btn-default btn-xs bg-green'>" + data +
                        "</button>";
                } else if (data === "revisi") {
                    stt += "<button type='button' class='btn btn-default btn-xs bg-orange'>" + data +
                        "</button>";
                }
...
function changeStatusPembelajaran() {
    ...
    $.post("controller/data_materi_keterampilan.php", {
        act: "changeStatus",
        id: id,
        status: status
    }, function(data) {
...
function resetField() {
    ...
    $("#kelas_bidang_studi").val("0").trigger("change");
    $("#document").val("");
...
function edit(id) {
    ...
    $.post("controller/data_materi_keterampilan.php", {
        ...
        $("#kelas_bidang_studi").val(dataSet.kelas_bidang_studi).trigger("change");
...
function set_option() {
    $.post("controller/data_materi_keterampilan.php", {
        ...
        $(data.jurusan).each(function(i, val) {
            opt_jurusan += "<option value='" + val.id + "'>[" + val.kode_jurusan + "] " + val
                .nama_jurusan + "</option>";
        });
        ...
        $("#kelas_bidang_studi").html(opt_kelas);
        $("#txtFilterJurusan").html(opt_jurusan);
            ...
            success: function(data) {
                if (data.status === false) {
                    if (data.error_code === "over_size") {
                        ...
                        alert(
                            "Dokumen yang boleh di upload adalah *.doc, *.docx, *.xls, *.xlsx, *.ppt, *.pptx"
                            );
                    ...
                    var confirm = window.open(data.callback_uri, "",
